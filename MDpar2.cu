...
double transpostaV[3][N];


void transpose(double A[][3], double B[3][N]);
...
void PotentialEcomputeAccelerations();
...
int main()
{
    ...
    transpose(r, transpostaR);
    transpose(a, transpostaA);
    transpose(v, transpostaV);
    ...
    for (i=0; i<NumTime+1; i++) {
        ...
        double array;
        array = VelocityVerlet(dt, i+1, tfp, &PE);
        Press = array*PressFac;
        //  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
double MeanSquaredVelocity() { 
    ...
    for (int i=0; i<N; i++) {
        ...
        vx2 = vx2 + transpostaV[0][i]*transpostaV[0][i];
        vy2 = vy2 + transpostaV[1][i]*transpostaV[1][i];
        vz2 = vz2 + transpostaV[2][i]*transpostaV[2][i];
...
double Kinetic() { //Write Function here!  
    ...
    for (int i=0; i<N; i++) {
        ...
        v2 = 0.;
        for (int j=0; j<3; j++) {
            v2 += transpostaV[j][i]*transpostaV[j][i];
        }
        kin += m*v2/2.;
...
//double contaPotencial(double r2){

//}


double contaComputeAccelerations(double rSqd){
    double invSqd = 1/rSqd;
    double invHelp = invSqd * invSqd;
    double invHelp2 = invHelp * invHelp;

    return  24 * (2*invHelp2*invHelp*invSqd - invHelp2);
}
...
void potentialComputeKernel(double *transpostaA1, double *transpostaA2, double *transpostaA3, double *transpostaR1, double *transpostaR2, double *transpostaR3, int tam, double *PotF){
    ...
    if (id < tam-1) { // -1 ??
        ...
        for (int j=id+1; j<tam; j++){
            ...
            atomicAdd2(&transpostaA1[j], -conta1);
            atomicAdd2(&transpostaA2[j], -conta2);
            atomicAdd2(&transpostaA3[j], -conta3);
        }
        //transpostaA1[i] = b;
        //transpostaA2[i] = c;
        //transpostaA3[i] = d;

        atomicAdd2(&transpostaA1[id], b);
        atomicAdd2(&transpostaA2[id], c);
        atomicAdd2(&transpostaA3[id], d);
    }
    atomicAdd2(PotF, Pot);
    //return 2*Pot;
}
...
void computeAccelerations() {
    ...
    // #pragma omp parallel for reduction(+:transpostaA[:3][:N]) schedule(dynamic,50) 
    for (i = 0; i < N-1; i++) {   // loop over all distinct pairs i,j
        ...
        for (j = i+1; j < N; j++) {
            ...
            rij[0] = g - transpostaR[0][j];
            rSqd += rij[0] * rij[0];

            rij[1] = h - transpostaR[1][j];
            rSqd += rij[1] * rij[1];

            rij[2] = e - transpostaR[2][j];
            rSqd += rij[2] * rij[2];

            f = contaComputeAccelerations(rSqd);
            ...
            transpostaA[0][j] -= conta1;
            transpostaA[1][j] -= conta2;
            transpostaA[2][j] -= conta3;
        }   
...
double VelocityVerlet(double dt, int iter, FILE *fp, double* Pot) {
    ...
    for (i=0; i<N; i++) {
        for(k=0; k<3; k++){
            transpostaR[k][i] += transpostaV[k][i]*dt + 0.5*transpostaA[k][i]*dt*dt;
            transpostaV[k][i] += 0.5*transpostaA[k][i]*dt;
        }
        //printf("  %i  %6.4e   %6.4e   %6.4e\n",i,r[i][0],r[i][1],r[i][2]);
    }

    //  Update accellerations from updated positions
    PotentialEcomputeAccelerations(&Pot);
    ...
    for (i=0; i<N; i++) {
        for (k=0;k<3;k++){
            if (transpostaR[k][i]<0. || transpostaR[k][i]>=L) {
                transpostaV[k][i] *=-1.; //- elastic walls
                psum += 2*m*fabs(transpostaV[k][i])/dt;  // contribution to pressure from "left" walls
            }         
        }
    }
